  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pets.map((pet) => (
          <div key={pet.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-pink-900 p-6 transition-all duration-200 hover:shadow-md hover:scale-105">
            <div className="flex items-center space-x-4">
              <div className="w-12 h-12 bg-green-100 dark:bg-teal-900 rounded-full flex items-center justify-center">
                <PawPrint className="w-8 h-8 text-pink-600 dark:text-pink-300" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-teal-800 dark:text-pink-100">{pet.name}</h3>
